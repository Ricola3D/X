...
*/
X.renderer3D.prototype.pick3d = function(x, y, withObject, withNormals, withVisibility, usePick2d) {
  ...
  var result = new Array(); // the buffer for the resulting points : several intersection can be found depending of the mesh quality and the parameters
    ...
    for (var i in objects) {
      ...
      if (!this.intersectionRayAABB(camera_center, direction, object)) {
        ...
        for (var j=0 ; j<length ; j++) {
          ...
          var w0 = new goog.math.Vec3(camera_center[0] - x[0],camera_center[1] - x[1],camera_center[2] - x[2]);
  ...
  if (nb_points==0) return new Array();
    ...
    for (var p=1 ; p<nb_points ; p++) {
      var sqDistNearestPoint = (camera_center[0]-nearest[0])*(camera_center[0]-nearest[0]) + (camera_center[1]-nearest[1])*(camera_center[1]-nearest[1]) + (camera_center[2]-nearest[2])*(camera_center[2]-nearest[2]);
      var sqDistThisPoint = (camera_center[0]-result[p][0])*(camera_center[0]-result[p][0]) + (camera_center[1]-result[p][1])*(camera_center[1]-result[p][1]) + (camera_center[2]-result[p][2])*(camera_center[2]-result[p][2]);
